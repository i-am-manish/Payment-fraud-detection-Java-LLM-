...
public class RetrievalService 
{
	...
	{	
		...
		{	
			...
			contextType = getContextType(contextType, text);
		}
	...
	private String getContextType(String contextType, String text) 
	{	
		String result = contextType;
		
		// Customer John Mayo has a card transaction		
		if(contextType == null || "".equals(contextType))
		{
			result = text;
			result = result.substring(0, result.indexOf("has")); // Customer John Mayo 
			result = result.replace("Customer", ""); //  John Mayo
			result =  result.trim();// John Mayo
			return result;
		}
		return result ;
	}
